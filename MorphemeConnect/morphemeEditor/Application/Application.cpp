...
void Application::EventTrackWindow(int* current_frame, int* first_frame, float* zoom_level, bool* is_expanded)
{
	ImGui::Begin("EventTrack");
	{
		...
		if (this->m_morphemeSystem.GetCharacterDef() && this->m_morphemeSystem.GetCharacterDef()->isLoaded())
		{
			...
			ImSequencer::Sequencer(&m_eventTrackEditor, current_frame, &this->m_eventTrackEditorFlags.m_selectedTrack, &this->m_eventTrackEditorFlags.m_selectedEvent, is_expanded, focused, first_frame, zoom_level, ImSequencer::EDITOR_EDIT_ALL | ImSequencer::EDITOR_TRACK_RENAME | ImSequencer::EDITOR_MARK_ACTIVE_EVENTS);
			ImGui::EndChild();
		}
	}
	ImGui::End();
...
bool Application::ExportAnimationToFbx(std::filesystem::path export_path, int anim_id)
{
	...
	if (this->m_fbxExportFlags.m_exportModelWithAnims)
	{
		if (!FBXTranslator::CreateFbxModel(pScene, this->m_animPlayer.GetModel(), characterDef->getNetworkDef()->getRig(0), this->m_chrId, pBindPoses, pMorphemeRig, export_path, this->m_animPlayer.GetFlverToMorphemeBoneMap()))
		{
...
bool Application::ExportModelToFbx(std::filesystem::path export_path)
{
	...
	if (!FBXTranslator::CreateFbxModel(pScene, this->m_animPlayer.GetModel(), characterDef->getNetworkDef()->getRig(0), this->m_chrId, pBindPoses, pMorphemeRig, model_out, this->m_animPlayer.GetFlverToMorphemeBoneMap()))
	{
