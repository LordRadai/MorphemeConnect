...
//Zeroes the translation components of a world matrix
DirectX::XMMATRIX Math::GetRotationMatrixFromWorldMatrix(DirectX::XMMATRIX m_world)
{
...
//Computes rotation matrix of the angle between pointA and pointB
DirectX::XMMATRIX Math::GetRotationFrom2Vectors(DirectX::SimpleMath::Vector3 pointA, DirectX::SimpleMath::Vector3 pointB)
{
...
//Gets euler angles from a rotation matrix
DirectX::SimpleMath::Vector3 Math::GetYawPitchRollFromMatrix(DirectX::XMMATRIX m_world)
{
...
//Gets LookAt vector from a world matrix
DirectX::SimpleMath::Vector3 Math::GetLookAtVectorFromMatrix(DirectX::XMMATRIX world)
{
	DirectX::XMMATRIX rotation = Math::GetRotationMatrixFromWorldMatrix(world);
...
DirectX::SimpleMath::Vector3 Math::CovnertQuatToEulerAngles(const DirectX::SimpleMath::Quaternion& quaternion)
{
...
inline float Math::ConvertDegAngleToRad(float angle)
{
	return ((angle / 360.f) * XM_2PI);
}

//Converts radians to degrees
inline float Math::ConvertRadAngleToDeg(float angle)
{
...
int Math::TimeToFrame(float time, int frameRate, bool round)
{
...
//Extracts the bits from an integer value
UINT Math::ExtractBits(int value, int shiftValue, int mask)
{
