...
void DrawPinShape(const ImVec2& pin_pos, const ImPinData& pin, const ImU32 pin_color)
{
    ...
    case ImNodesPinShape_Flow:
    {
        for (int i = 0; i < 3; i++)
        {
            ImVec2 relative_offset;

            if (pin.Type == ImNodesAttributeType_Output)
                relative_offset = ImVec2(-i * 1.5 * GImNodes->Style.PinCircleRadius, 0);
            else
                relative_offset = ImVec2(i * 1.5 * GImNodes->Style.PinCircleRadius, 0);

            if (i < 2)
                GImNodes->CanvasDrawList->AddCircleFilled(
                    pin_pos + relative_offset, 0.5 * GImNodes->Style.PinCircleRadius, 0xFF00FF00, CIRCLE_NUM_SEGMENTS);
            else
                GImNodes->CanvasDrawList->AddCircleFilled(
                    pin_pos + relative_offset, 0.5 * GImNodes->Style.PinCircleRadius, pin_color, CIRCLE_NUM_SEGMENTS);
        }
    }
    break;
    default:
